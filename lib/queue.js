...
Queue.prototype.use = function (store, opts) {
  ...
  var loadStore = function (store) {
    var Store;
    try {
      Store = require('@umbrella.limited/better-queue-' + store);
    } catch (e) {
      throw new Error('Attempting to require @umbrella.limited/better-queue-' + store + ', but failed.\nPlease ensure you have this store installed via npm install --save @umbrella.limited/better-queue-' + store)
    }
    return Store;
  }
